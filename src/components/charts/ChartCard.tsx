  ...
  return (
    <section style={{ borderRadius: 18, border: '1px solid hsl(var(--border))', background: 'hsl(var(--card))', padding: '14px 16px 10px' }}>
      <header style={{ marginBottom: 10, display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
        <h3 style={{ fontFamily: '"DM Serif Display", Georgia, serif', fontSize: 22, fontWeight: 400, letterSpacing: -0.4, lineHeight: 1, margin: 0 }}>{title}</h3>
        {hint && <span style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.16em', color: 'hsl(var(--muted-foreground))' }}>{hint}</span>}
      </header>
      {empty ? (
